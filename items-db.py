...
url = "http://www.roguard.net/db/items/"
...
def mon_urlList(x):      
    ...
    df = pd.DataFrame(index=ch_list,data=mon_list,columns=['Item Name'])
    return df
...
writer = pd.ExcelWriter('output_itemList.xlsx')
full_df.to_excel(writer,'Item List')
writer.save()
print full_df